#include "texturesplattering.h"
#include <QFileDialog>
 
void TextureSplattering::onPluginLoad()
{
    GLWidget &g = *glwidget();
    g.makeCurrent();
	// VS
    ...
    QString fs_src =
      ...
      "                 texture(sampler1, vtexCoord),"
      "                 texture(sampler2, vtexCoord)); }";
	...
	// the third texture is gonna contain the mixing value between texture 1 and texture 2
...
void TextureSplattering::preFrame() 
{
    GLWidget &g = *glwidget();
    g.makeCurrent();
...
void TextureSplattering::postFrame() 
{
    GLWidget &g = *glwidget();
    g.makeCurrent();
...
Q_EXPORT_PLUGIN2(texturesplattering, TextureSplattering)   // plugin name, plugin class
#endif 
